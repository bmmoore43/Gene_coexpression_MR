...
# Define function for calculating SCC and MR
print("Calculating SCCs...") 
def fork_scc(geneList):
    for gid1 in geneList:
        sccs = {}
        for gid2 in range(0, numgenes):
            corr,_ = spearmanr(expDict[gid1], expDict[gid2])
            #print(gene1,gene2,corr)
            sccs[gid2] = corr
        sorted_d = sorted(sccs.items(), key=operator.itemgetter(1), reverse = True)
...
for i in range(threads):
    ...
    if pid == 0:
        ...
        print("  thread", tct, ": Calculating SCCs for", len(geneList), 'genes...') 
        fork_scc(geneList)
        exit()

for i in range(threads):
    finished = os.waitpid(0, 0)
print("Finished calculating SCCs!\n\n") 


# read in sccs and store sccs and ranks in np arrays
...
for file in files:
    ...
    for line in fi:
        [gid2, scc] = line.rstrip().split('\t')
        gid1 = int(gid1)
        gid2 = int(gid2)
        scc = np.round(float(scc),3)
...
def fork_out(geneList):
	for gid1 in geneList:
		...
		for gm in sorted_d:
			...
			line = gene2 + '\t' + str(np.round(mr,3)) + '\t' + str(sccDict[gid2]) + '\n'
			fo.write(line)
